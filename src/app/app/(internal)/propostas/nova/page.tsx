import Link from "next/link"
import { ArrowLeft } from "lucide-react"
import { criarProposta } from "@/lib/actions"
import { listarClientes } from "@/lib/data"
import { SubmitButton } from "@/components/internal/SubmitButton"

export const metadata = { title: "Nova proposta" }

const inputCls =
  "h-10 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus-visible:border-ring"
const labelCls = "text-sm font-medium"

export default async function NovaPropostaPage() {
  const clientes = await listarClientes()

  return (
    <div className="mx-auto max-w-2xl">
      <Link href="/app/propostas" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" /> Propostas
      </Link>
      <h1 className="mt-3 font-heading text-2xl font-semibold tracking-tight">Nova proposta</h1>

      <form action={criarProposta} className="mt-6 space-y-5 rounded-xl border border-border bg-card p-6 shadow-xs">
        <label className="block space-y-1.5">
          <span className={labelCls}>Título *</span>
          <input name="titulo" required placeholder="Ex.: Desenvolvimento do app + 3 meses de suporte" className={inputCls} />
        </label>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="space-y-1.5">
            <span className={labelCls}>Cliente *</span>
            <select name="cliente_id" required defaultValue="" className={inputCls}>
              <option value="" disabled>Selecione…</option>
              {clientes.map((c) => (
                <option key={c.id} value={c.id}>{c.nome}</option>
              ))}
            </select>
          </label>
          <label className="space-y-1.5">
            <span className={labelCls}>Válida até</span>
            <input name="validade" type="date" className={inputCls} />
          </label>
        </div>
        <label className="block space-y-1.5">
          <span className={labelCls}>Itens</span>
          <textarea
            name="itens"
            rows={5}
            placeholder={"Um item por linha, no formato:\nDescrição do item | 5000\nManutenção mensal | 1200"}
            className={`${inputCls} h-auto py-2 font-mono text-xs`}
          />
          <span className="text-xs text-muted-foreground">Uma linha por item: <code>descrição | valor</code>. O total é somado automaticamente.</span>
        </label>

        <div className="flex justify-end gap-2 border-t border-border pt-5">
          <Link href="/app/propostas" className="rounded-lg border border-border px-4 py-2 text-sm font-medium hover:bg-muted">
            Cancelar
          </Link>
          <SubmitButton className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-xs transition-opacity hover:opacity-90 disabled:opacity-60">
            Criar proposta
          </SubmitButton>
        </div>
      </form>
    </div>
  )
}
